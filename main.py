import discordimport osimport randomimport asyncioimport functoolsimport loggingfrom dotenv import load_dotenvfrom discord.ext import commandsimport yt_dlpfrom collections import deque
...
YDL_OPTIONS = {# m4a is usually more stable for Discord/FFmpeg than some webm/opus streams'format': 'bestaudio[ext=m4a]/bestaudio/best','noplaylist': True,'quiet': True,'no_warnings': True,'default_search': 'ytsearch','cachedir': False,}

FFMPEG_OPTIONS = {'before_options': ('-nostdin ''-reconnect 1 ''-reconnect_streamed 1 ''-reconnect_at_eof 1 ''-reconnect_on_network_error 1 ''-reconnect_delay_max 10'),'options': '-vn'}

IDLE_LEAVE_SECONDS = 180ALONE_LEAVE_SECONDS = 30
...
def format_duration(seconds):"""Converts seconds into a nice mm or hh:mm format."""try:if not seconds:return 'Unknown'
...
def search_youtube(query):"""Searches YouTube and returns song info. Runs in a thread so it doesn't block the bot."""try:if not query or not query.strip():return None
...
class MusicState:"""Holds all music-related state for a single Discord server."""
...
class MusicBot(commands.Bot):"""Bot subclass with per-guild music states."""
...
intents = discord.Intents.default()intents.message_content = Trueintents.voice_states = True
...
@bot.eventasync def on_ready():"""Runs when the bot successfully connects to Discord."""bot.loop_ref = asyncio.get_running_loop()logger.info(f'Logged on as {bot.user}!')

@bot.eventasync def on_voice_state_update(member, before, after):"""Auto-leave when the bot is alone in a voice channel."""try:if member == bot.user:return
    ...
    if len(bot_channel.members) == 1:
        ...
        async def alone_disconnect():
            try:
                ...
                if len(current_state.voice_client.channel.members) > 1:
                    return
...
@bot.command(name='play')async def play(ctx, *, query: str = ''):"""Search YouTube and play a song. Adds to queue if something is already playing."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='np')async def now_playing(ctx):"""Show details about the song currently playing."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='loop')async def loop_cmd(ctx, *, arg: str = ''):"""Set loop mode: song, queue, or off. No argument shows current status."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='skip')async def skip(ctx):"""Skip to the next song in the queue."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='queue')async def queue_cmd(ctx):"""Show the current song and upcoming queue."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='stop')async def stop(ctx):"""Stop playback and clear the queue."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='pause')async def pause(ctx):"""Pause the current song."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='resume')async def resume(ctx):"""Resume a paused song."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='leave')async def leave(ctx):"""Disconnect the bot from voice and clear the queue."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='shuffle')async def shuffle(ctx):"""Randomize the order of songs in the queue."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='volume')async def volume_cmd(ctx, *, arg: str = ''):"""Set the playback volume 0-100. No argument shows current volume."""try:if not ctx.guild:return await ctx.send('❌ This command can only be used inside a server.')
...
@bot.command(name='help')async def help_cmd(ctx):"""Show the list of available commands."""help_text = ('🎵 Music Bot Commands\n\n''!play <search> — Search and play a song\n''!pause — Pause the current song\n''!resume — Resume playback\n''!stop — Stop and clear the queue\n''!skip — Skip to next song\n''!queue — Show the queue\n''!np — Now playing info\n''!loop song/queue/off — Set loop mode\n''!shuffle — Shuffle the queue\n''!volume <0-100> — Set volume\n''!leave — Disconnect the bot\n''!help — Show this list')
...
def main():token = os.getenv('DISCORD_TOKEN')
